...
void log_inbound_message(uint8_t message_index, int port){
    ...
    buffer[i+j] = 0;
    //sprintf(log_buffer, "%s:: buffer: <%s>, i+j: %d, pdr: %d, port: %d, m_payload: <%s>, m_len: %d, crc_b: <%s>.", __func__, buffer, i+j, pdr, port, new_message->Payload, message_len, new_message->CRC_buffer);
    //logger(log_buffer, 4);
