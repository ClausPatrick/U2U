...
void print_queue(struct Message_Queue* queue);
...
//void write_to_port0(char* buffer, size_t length);
//void write_to_port1(char* buffer, size_t length);
//void write_to_port2(char* buffer, size_t length);
...
void print_segments(struct Message* message, uint8_t port);
void print_message_segments(uint8_t port);
...
uint8_t u2u_self_test_from_file(uint8_t port);
void messages_init();
