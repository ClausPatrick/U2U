...
int u2u_topic_exchange(int topic_nr, const char* buffer, size_t length){
    ...
    for (size_t c=0; c<size; c++){
        char ch = buffer[c];
        topic_response_buffers[topic_nr][c] = ch;
        if (ch == 0){
            break;
        }
    }
...
uint8_t general_call(struct Message* message, uint8_t port){
    char buffer[MAX_MESSAGE_SIZE];
    size_t length = response_composer(message, buffer);
    write_response_message(buffer, length, port);
    length = forward_composer(message, buffer);
    write_forward_message(buffer, length, port);
    return 0;
}
...
uint8_t other_call(struct Message* message, uint8_t port){
    char buffer[MAX_MESSAGE_SIZE];
    size_t length = forward_composer(message, buffer);
    write_forward_message(buffer, length, port);
    return 0;
}
...
int pipe_message_composer(uint8_t port, char* segments){
    ...
    if (custom_sender == false){
        ...
        buffer_index += add_segment(buffer, temp_buffer, buffer_index, temp_size);
    }
    ...
    // Compose message.
    if (custom_sender == false){
        buffer_index += add_segment(buffer, SELF_NAME, buffer_index, self_name_len);
    }
    for (size_t s=1; s<segment_counter; s++){
        buffer_index += add_segment(buffer, segment_pointers[s], buffer_index, segment_lengths[s]);
    }
    if (custom_hops == false){
        buffer[buffer_index++] = '0';    // HOPS set to 0
    }
    fill_length_preamble(buffer, buffer_index);
    ...
    switch (port){
        case 0:
            u2u_send_message_uart0(buffer, buffer_index);
            break;
        case 1:
            u2u_send_message_uart1(buffer, buffer_index);
            break;
        case 2:
            u2u_send_message_uart2(buffer, buffer_index);
            break;
        default:
            u2u_send_message_uart0(buffer, buffer_index);
            u2u_send_message_uart1(buffer, buffer_index);
